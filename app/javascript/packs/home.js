...
$(document).ready(function() {
    ...
    $(document).on('click', '.list-group-item', function() {
        ...
        if (interest.length == 5) {
            errorAlert();
        } else if (id.substring(0, 5) != "NoVal" && interest.length < 5) {
            if ($(".Intr" + id).length != 0) {
            ...
            $('.interest-buttons').append('<a class="IntrButton Intr' + newIntr + ' btn btn-primary mb-1 mr-1" valset="true" id="' + newIntr + '"> ' + newIntr + '</a>');
            var newIntrLower = newIntr.toLowerCase();
            updateInterest(newIntrLower, "add");
            ...
            if (!/[^a-zA-Z]/.test(interest[i])) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        sendPost2("http://localhost:3000/insertinterest", val, x);
                    }
                });
                continue;
